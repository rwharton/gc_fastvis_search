...
import sys
import time
import fastvis_params as params
import sifting
import re
import shlex
import psr_utils
...
def run_singlepulse_search(work_dir, w_max=params.max_width, \
                               ...
                               sp_flags=params.sp_otherflags):
    print("Looking for single pulses...\n")
    t_sp_start = time.time()
    params = "-m %s %s" %(str(w_max), sp_flags)
    cmd = "%s %s %s/*.dat" %(sp_exe, params, work_dir)
...
def setup_beam(bnum, tmp_dir, out_dir, top_dir, basename, part_list):
    ...
    # Copy FITS files to PROC dir if desired
    if copy_fits:
    ...
    if resume:
        ...
        for sub_dir in sub_dirs:
            if os.path.exists("%s/%s" %(bdir_res, sub_dir)):
                print("COPYING %s" %sub_dir)
                sub_res  = "%s/%s" %(bdir_res, sub_dir)
                sub_work = "%s/%s" %(bdir_work, sub_dir)
                shutil.copytree(sub_res, sub_work)
            else:
                pass

        # Unpack DM files (if applicable)
        dm_files = glob("%s/dedisperse/*" %(bdir_work))
        if len(dm_files):
            for dm_file in dm_files:
                shutil.move(dm_file, bdir_work)
            shutil.rmtree("%s/dedisperse" %(bdir_work))
...
def copy_beam_results_orig(bnum, tmp_dir, out_dir):
    """
...
def copy_beam_results(bnum, tmp_dir, out_dir):
    ...
    log_files = glob("%s/*log" %(work_dir))
    if len(log_files):
        for log_file in log_files:
            fname = log_file.split('/')[-1]
            shutil.copyfile(log_file, "%s/%s" %(res_dir, fname))
    else: pass
...
if __name__ == "__main__":
    ...
    for bnum in beam_list:
        ...
        try:
            # Set-up beam + paths in tmp space
            setup_beam(bnum, tmp_dir, search_dir, top_dir, bname, plist) 

            # Search beam 
            search_beam(basename, fits_dir, work_dir)

            # Copy results to search dir and clean up
            copy_beam_results(bnum, tmp_dir, search_dir)
        ...
        # Remove results on tmp space (all cases)
        finally:
            # Clean up beam (even if processing fails)
            remove_beam(bnum, tmp_dir)
